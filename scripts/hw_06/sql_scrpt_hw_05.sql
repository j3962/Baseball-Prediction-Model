...
select
    ...
    , if( sum(if(pc_right.startingPitcher = 1 and pc_right.home_team_wins = 1, 1, 0)) = 0 or count(pc_right.pitcher) = 0, 0, sum(if(pc_right.startingPitcher = 1 and pc_right.home_team_wins = 1, 1, 0)) / nullif(count(pc_right.pitcher), 0)) as starting_pitch_away_w
from
...
select
    ...
    , avg(batting_average_against) as avg_batting_average_against
    , avg(bb_9) as avg_bb_9
    , avg(strikeout_to_walk_ratio) as avg_strikeout_to_walk
    , avg(home_run_per_pitc) as avg_home_run_per_pitc
    , avg(ip_gs) as avg_ip_gs
    , avg(fielding_indp_pitchin) as fip
...
create index if not exists date_idx on pitcher_features (game_id);
...
select
    ...
    , apc_home.avg_strikeout_to_walk / nullif(apc_away.avg_strikeout_to_walk, 0) as avg_strikeout_to_walk_ratio
    , apc_home.avg_home_run_per_pitc / nullif(apc_away.avg_home_run_per_pitc, 0) as avg_home_run_per_pitc
    ...
    , apc_home.fip / nullif(apc_away.fip, 0) as avg_fip_ratio
    , apc_home.avg_bb_9 / nullif(apc_away.avg_bb_9, 0) as avg_bb_9
    , pf_home.hits_per_innings / nullif(pf_away.hits_per_innings, 0) as hits_per_innings_ration
    , apc_home.avg_ip_gs / nullif(apc_away.avg_ip_gs, 0) as avg_ip_gs
    ...
    , apc_home.avg_pitch_count / nullif(apc_away.avg_pitch_count, 0) as avg_pitch_count_ratio
    , apc_home.avg_whip / nullif(apc_away.avg_whip, 0) as avg_whip_ratio
    , apc_home.avg_batting_average_against / nullif(apc_away.avg_batting_average_against, 0) as avg_batting_average_against
    , gm_res.home_team_wins
from game_res gm_res
    ...
    left join avg_pitch_count apc_home on gm_res.game_id = apc_home.game_id and gm_res.home_team_id=apc_home.team_id
    left join avg_pitch_count apc_away on gm_res.game_id = apc_away.game_id and gm_res.away_team_id=apc_away.team_id
